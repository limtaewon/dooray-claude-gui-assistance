...
## [Unreleased] - Windows 호환 수복 기반 다지기 (feat/version-2.0 브랜치)
...
- **두레이 위키에서 받은 스킬 저장 실패 수정** — 위키 제목에 `:`, `/` 등 Windows 에서 파일명으로 쓸 수 없는 문자가 있으면 자동으로 안전한 이름으로 정제해 저장합니다. 스킬 삭제 시 빈 디렉터리가 남아 같은 이름으로 재저장할 때 꼬이던 문제도 함께 정리했습니다.
- **스킬/커맨드가 하나도 없는 신규 사용자에서 변경 감지가 죽어있던 문제 수정**.
- **MCP `npx` 기반 서버가 Windows 에서 등록해도 뜨지 않던 문제 수정** — 저장 시 자동으로 `cmd /c` 로 감싸 등록합니다(비활성화/재활성화를 반복해도 중복으로 감싸지 않음).
...
- **터미널 탭 드래그 순서 변경** — 탭을 12px 이상 끌면 순서를 바꿀 수 있습니다(더블클릭 이름 변경과 충돌하지 않도록 임계값을 둠). 탭을 닫으면 최근 사용한 탭이 우선 활성화됩니다. 재배치한 순서는 이름 변경과 같은 수준으로 저장되어 재시작 후에도 대체로 유지됩니다(완전한 영속화 보장은 후속 사이클 예정).
