...
- **두레이 업무에서 바로 작업 시작** — 업무를 고르고 `[▶ 작업 시작]` 한 번이면 워크트리 + 브랜치가 만들어지고 그 폴더에서 claude 가 실행됩니다. 버튼 아래에 `저장소 · base · 브랜치 · 에이전트 1` 요약이 미리 표시되고, 설정을 바꾸려면 `▾`(또는 `⌥클릭`)로 모달을 엽니다. 두레이 상태를 '진행중'으로 바꾸고 브랜치명을 댓글로 남기는 것도 옵션으로 함께 처리됩니다.
- **여러 업무를 동시에** — 업무마다 워크트리가 분리되어 여러 브랜치를 나란히 진행할 수 있습니다. 왼쪽 목록에 브랜치 배지와 상태 점(작업 중/응답 대기)이 표시되고 클릭으로 전환합니다.
- **터미널을 보면서 업무 본문 읽기** — 오른쪽 패널에서 업무 상세(본문·댓글·상태 변경)와 현재 워크트리의 미커밋 변경사항을 탭으로 오갑니다.
- **프롬프트를 비우면 자동 입력 없이 claude 만 실행** — 첫 지시를 터미널에서 직접 하는 방식도 지원합니다.
- **재연결 · 채택 · 정리** — 앱 재시작으로 끊긴 세션은 `재연결`로 `claude --resume` 이어하기, 끝난 작업은 `채택` 표시 후 `정리`로 워크트리를 제거합니다.
- ⚙ 설정 → **워크스페이스** 에서 저장소를 등록하고(저장소별 기본 base·프리픽스), 브랜치 이름 템플릿(라이브 미리보기)과 작업 시작 기본값을 조정합니다.

### 새 기능 — 터미널 태스크 드로어 (⌘⇧T)

- **업무를 터미널에 끌어다 놓기** — 터미널 탭바의 `📋 태스크` 로 두레이 업무 목록을 열고, 카드를 화면에 드롭하면 매핑된 저장소로 `cd` 한 뒤 claude 가 실행되며 업무 내용을 첫 지시로 전달합니다.
- **두 번째부터는 이어하기** — 한 번 작업한 업무에는 `세션 연결됨` 배지가 붙고, 다시 드롭하면 `claude --resume` 으로 이전 대화를 이어갑니다(프롬프트를 다시 넣지 않습니다). 배지 옆 아이콘으로 연결을 해제할 수 있습니다.
- 워크트리를 만들지 않는 가벼운 흐름이며, 세션 연결은 워크스페이스 뷰와 공유합니다.
