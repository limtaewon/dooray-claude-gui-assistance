...
### 추가 — 터미널에서 파일을 앱 안에서 바로 열고 고칩니다

지금까지 터미널의 경로를 ⌘클릭하면 **전부 앱 밖으로** 나갔습니다. 파일도 URL도 OS 기본 앱에
넘겼습니다. 그래서 로그에 찍힌 파일 하나 보려고 매번 다른 앱으로 건너가야 했습니다.

이제 텍스트 파일은 **앱 안 탭**으로 열립니다.

- 코드·문서·설정 파일을 탭으로 열고 **⌘S 로 저장**합니다
- `파일.ts:120` 형태면 그 줄로 바로 갑니다
- 같은 파일을 또 누르면 탭을 새로 만들지 않고 그 탭으로 갑니다
- 저장 전에 그 사이 파일이 바뀌었으면 **덮어쓰지 않고 알려줍니다** — 터미널에서 돌린
  스크립트나 git 이 같은 파일을 건드리는 일이 흔한 화면이라서입니다

마크다운(`.md`)과 HTML 은 **렌더된 모습으로 먼저** 열립니다. 문서를 열었는데 태그부터 보이면
한 번 더 눌러야 하니까요. 「소스」 버튼으로 원문 편집으로 넘어갑니다. 그 외 형식에는 토글이
아예 안 나옵니다 — 눌러도 아무 일 없는 버튼을 두지 않았습니다.

HTML 미리보기 안에서 **목차 링크를 눌러 문서 안을 이동**할 수 있고, 문서가 참조하는 CSS·이미지도
함께 뜹니다. 문서 안의 스크립트는 실행되지 않습니다 — 미리보기는 읽기 위한 화면입니다.

앱이 그릴 수 없는 것(폴더 · 이미지 · 2MB 넘는 파일)은 지금처럼 OS 기본 앱으로 넘어갑니다.
**⌥⌘클릭은 언제나 OS 기본 앱**입니다 — 늘 쓰던 편집기로 여는 길을 막지 않았습니다.

### 수정 — 터미널에서 한글 폴더·파일 경로를 못 잡았습니다

터미널 출력의 경로를 ⌘(Ctrl)+클릭으로 여는 기능이 **한글이 섞이면 거기서 끊겼습니다.**
`/Users/nhn/문서/보고서.md` 는 `/Users/nhn/` 까지만 인식했고, `문서/한글파일.txt` 처럼
한글로 시작하는 상대 경로는 아예 못 잡았습니다.

경로 인식에 쓰는 글자 범위가 영문·숫자로 한정돼 있던 탓입니다. 이제 한글은 물론 일본어·중국어·
악센트 문자(`résumé`)까지 인식합니다. macOS 가 파일명을 자모 분리 형태로 돌려주는 경우도
함께 처리합니다.

### 수정 — 메신저 연결이 「연결 중…」에서 멈춰 있었습니다

두레이 메신저 실시간 연결이 붙을 때도 있고, 「CONNECTING」 상태로 계속 머무는 때도 많았습니다.
끊기면 자동으로 다시 붙는다고 안내하면서 실제로는 다시 붙지 않았습니다.

재연결은 **소켓이 닫히는 것을 신호로** 다음 시도로 넘어갑니다. 그래서 소켓이 닫히지도, 열리지도
않는 상태에 빠지면 그 자리에서 영원히 기다렸습니다. 그런 구간이 넷 있었습니다.

- 접속 토큰 요청이 응답하지 않을 때
- 연결 협상이 끝나지 않을 때
- 연결은 됐는데 서버가 세션 승인을 보내지 않을 때
- 회선이 조용히 끊겨 소켓만 살아 있을 때 (무선 전환·VPN 재접속·절전 복귀)

특히 서버가 협상을 **거절**했을 때(토큰 만료 등)가 잦은 원인이었습니다. 거절 응답을 받고도
연결을 정리하지 않아 "닫힘" 신호가 영영 오지 않았습니다.

이제 네 구간 모두에 제한 시간을 두고, 걸리면 연결을 끊어 다시 붙습니다. 재시도 간격도
연결에 성공하면 처음으로 되돌립니다 — 안 그러면 끊길 때마다 간격이 늘어 나중에는 한 번
붙는 데 30초씩 걸렸습니다.

### 개선 — 터미널 탭바 버튼이 무슨 기능인지 안 보였습니다

탭바 오른쪽에 아이콘만 있는 버튼이 셋 있었는데, 눌러보기 전에는 뭘 하는지 알 수 없었습니다.
특히 「↗」 버튼은 **지금 터미널이 있는 폴더를 IDE 로 여는** 기능인데 아이콘만으로는
새 창 열기·공유·바깥 링크 중 무엇인지 구분되지 않았습니다.

이제 글자로 말합니다.

- **분할** — 「분할」 라벨 뒤에 방향 아이콘 둘. 왼쪽이 오른쪽으로(⌘D), 오른쪽이 아래로(⌘⇧D).
  라벨을 셋으로 늘리면 창이 좁을 때 탭 영역을 다 먹어서, 방향만 아이콘에 남겼습니다.
- **에디터로 열기** — 감지된 에디터 이름을 그대로 씁니다. 하나면 「VS Code 로 열기」,
  여러 개면 「에디터로 열기」 를 누르고 고릅니다.

분할과 에디터 열기는 안내 투어에도 단계를 넣었습니다. 분할 단계는 원래 가리킬 대상이 없어
화면 가운데 카드로만 떴는데, 이제 실제 버튼을 가리킵니다.

### 수정 — 새로 나눈 터미널이 백지로 떠서 Enter 를 쳐야 프롬프트가 나왔습니다

터미널을 split 으로 나누면 새 pane 이 **아무것도 없는 채로** 떴습니다. 셸은 멀쩡히 살아 있어서
Enter 를 한 번 치면 그제야 프롬프트가 나타났습니다.

셸이 찍은 첫 프롬프트를 화면이 못 받는 것이 원인이었습니다. 터미널 프로세스는 만들어지는 즉시
출력을 뱉는데, 그 출력을 받을 화면 쪽 구독은 조금 뒤에 붙습니다. 그 사이에 나온 출력은
앱 안쪽(main)에는 쌓여 있었지만 화면에는 전달되지 않았습니다.

이제 pane 이 붙을 때 앱 안쪽에 쌓인 출력을 받아 **따라잡고** 시작합니다. 청크마다 번호를 매겨
따라잡은 지점 이후의 것만 이어 쓰므로 같은 출력이 두 번 찍히지 않습니다.

### 수정 — 터미널을 여러 개로 나누면 먼저 열어둔 쪽 글자가 깨졌습니다

터미널을 split 으로 나눌수록 **먼저 열어둔 pane** 의 글자가 조각나거나 아예 사라졌습니다.
새로 연 pane 만 멀쩡하고, 나머지는 획 일부만 남은 채 남아 있었습니다.

원인은 글리프 캐시(글자 비트맵 모음)를 pane 들이 **하나로 공유**한다는 점이었습니다. 새 pane 이
뜰 때마다 이 공유 캐시를 비웠는데, 비운 pane 만 화면을 다시 그리고 나머지는 그리지 않아서
이미 화면에 있던 글자들이 사라진 캐시의 옛 위치를 계속 가리켰습니다.

이제 캐시를 비우면 살아 있는 pane 을 **전부** 다시 그립니다. 그리고 글꼴 로딩이 이미 끝난
뒤에 열린 pane 은 애초에 캐시를 비우지 않습니다 — 비울 이유가 없는데 비우고 있었습니다.

### 수정 — 앱을 켜둔 채로는 새 버전을 몰랐습니다

업데이트 확인이 **앱 시작 직후 딱 한 번**만 돌았습니다. 그래서 앱을 켜둔 사이에 릴리즈가
나가면 그 세션 내내 알림이 뜨지 않고, 껐다 켜야만 알게 됐습니다 — 며칠씩 켜두는 앱이라
실제로 새 버전을 놓쳤습니다.

이제 6시간마다 다시 확인합니다. 받는 중이거나 이미 받아둔 상태, 「다시 시도」가 떠 있는
상태에서는 건너뜁니다 — 뒤에서 도는 확인이 진행 중인 표시를 덮어쓰면 안 되기 때문입니다.
이미 업데이트 버튼이 떠 있을 때도 건너뜁니다(알릴 것은 이미 알렸고, 다시 확인하면 조회하는
동안 버튼이 깜빡 사라졌다 돌아옵니다).

## [2.0.5] - 2026-08-04

작업 패널의 두레이 업무 목록을 손봤습니다 (이슈 #35).

### 추가 — 업무 정렬과 "그 뒤로 바뀐 것" 표시

기본 정렬이 **최근 변경순**이 됐습니다. 두레이는 댓글·상태 변경·본문 수정을 모두 `updatedAt`
갱신으로 남기므로, 지금 말이 오가는 업무가 위로 올라옵니다. 등록순 · 마감 임박순 · 상태순으로도
세울 수 있고 고른 순서는 앱을 껐다 켜도 유지됩니다.

지난번에 본 뒤 뭔가 바뀐 업무에는 카드 왼쪽 위에 점이 붙습니다. 업무를 열면 그 업무만,
갈래 칩 줄의 `모두 읽음` 을 누르면 전부 지워집니다. 판정은 이미 받아온 `updatedAt` 비교라
추가 API 호출이 없습니다 — 다만 무엇이 바뀌었는지(댓글인지 상태인지)는 열어야 보입니다.

목록을 받아오는 순서도 등록순에서 최근 변경순으로 바꿨습니다. 프로젝트당 200건까지만
받으므로, 오래된 프로젝트에서 지금 활발한 업무가 잘려나가던 것을 함께 고쳤습니다.

### 추가 — 카드에서 태그가 보이고, 눌러서 바로 좁혀집니다

전에는 상세 검색 팝오버에서 태그를 고를 수는 있었지만 **카드에는 태그가 보이지 않았습니다.**
이제 카드에 태그가 3개까지 나오고(나머지는 `+N`), 누르면 그 태그의 업무만 남습니다.
프로젝트가 달라도 이름이 같은 태그는 함께 묶입니다.

### 추가 — 업무의 스크린샷을 claude 에게 넘길 수 있습니다

업무를 터미널에 놓을 때 본문과 댓글의 첨부 이미지를 내려받아 첫 지시에 **파일 경로**로 함께
넘깁니다. 지금까지는 본문 글자만 전달돼, 재현 화면이 `![...](/files/123)` 이라는 글자로만
가서 claude 가 그림을 못 봤습니다. QA 업무처럼 스크린샷이 본문의 절반인 경우에 특히 큽니다.

한 업무당 8장까지 받고, 실패한 장은 조용히 건너뜁니다.

**켜는 방법**: 설정 → 워크스페이스의 첫 지시 문구에 `{images}` 를 넣으세요. 기본 문구는
그대로 두었습니다 — 이미지가 없는 업무까지 매번 본문·댓글을 조회하게 되어 드롭이 느려지고,
무엇보다 저장된 설정이 기본값을 덮는 구조라 기본 문구를 바꿔도 **신규 설치에만** 닿습니다.

### 수정 — `표시 중인 프로젝트` 가 고장난 것처럼 보이던 문제

작업 패널의 이 팝오버는 조회 전용입니다 — 프로젝트를 고르면 저장소·브랜치 이름·첫 지시 문구를
이어서 정해야 하는데, 그 자리가 설정에만 있기 때문입니다. 그런데 **체크박스는 그대로 그려놓고
`disabled` 만 걸어둬서**, 눌리게 생긴 것이 안 눌리니 고장으로 읽혔습니다.

잠그는 결정은 그대로 두고, 잠긴 것이 보이도록 고쳤습니다.

- 체크박스를 아예 그리지 않습니다 — 켜져 있는 것만 나오는 목록이라 체크 표시가 군더더기였습니다
- 헤더에 `고르는 곳은 설정입니다 — 프로젝트마다 저장소·브랜치 이름을 함께 정해야 합니다` 한 줄
- 하단 설정 링크가 이 팝오버의 유일한 동작이므로 흐린 부록이 아니라 또렷하게 보입니다

### 수정 — 업무의 이미지가 일부만 뜨던 문제

한 업무 안에서 어떤 스크린샷은 보이고 어떤 것은 `두레이에서 보기` 로 떨어지던 문제입니다.
경로도 권한도 멀쩡했고, 원인은 **두레이 파일 API 의 rate limit** 이었습니다. 응답 헤더에
그대로 적혀 있습니다 — 버킷 20개, 초당 5개 충전. 상세를 열면 본문·댓글의 이미지가 한꺼번에
요청되니, 버킷을 넘긴 뒤에 선 것부터 429 로 떨어지고 그대로 깨진 채 남았습니다.

세 가지를 고쳤습니다.

- **요청 수를 절반으로.** `api.dooray.com` 의 파일 경로는 307 로 `file-api.dooray.com` 을
  가리킬 뿐이라 이미지 한 장에 요청이 두 번 나갔습니다. 처음부터 실물 서버로 갑니다.
- **동시에 3장까지만** 받습니다(충전 속도보다 낮게).
- **429·5xx 는 실패가 아니라 재시도**입니다 — 250 → 500 → 1000ms. 한 번 걸렸다고 새로고침
  말고는 되살릴 방법이 없던 것을 없앴습니다.

권한이 없는 파일(다른 프로젝트에서 본문을 복사해 온 경우)은 403 이라 재시도하지 않고 바로
접습니다 — 어느 경로로도 못 받는데 남은 후보를 두드리면 버킷만 축냅니다.

## [2.0.4] - 2026-08-03

### 변경 — 새 브랜드 마크

크림색 배경의 네잎클로버에서 **주황→파랑 그라디언트 배경 위 방사형 마크**로 바꿨습니다.
마크 색이 앱 안의 AI 그라디언트와 같은 조합이라, 아이콘과 화면이 같은 언어를 씁니다.

앱 아이콘(macOS·Windows)과 타이틀바 로고가 같은 좌표를 공유합니다 — 한쪽만 바뀌면
로고가 갈리기 때문에 획 좌표를 코드에 명시해 두었습니다. 원본 SVG 는 `docs/brand/` 에 있습니다.

### 변경 — README 전면 교체

실제 화면을 v2.0.4 토큰으로 재조판한 목업 11장으로 갈아끼웠습니다(`docs/readme/`).
직접 찍은 캡처와 달리 창 크기·주변 앱·개인 데이터가 섞이지 않고, 토큰이 바뀌면 같이 다시 뽑을 수 있습니다.
v1.5.5 시절 스크린샷 참조는 전부 걷어냈습니다 — 이번 디자인 변경으로 실제와 달라졌기 때문입니다.

### 추가 — 새 버전이 나오면 앱이 알려줍니다

지금까지는 릴리즈 페이지를 직접 들여다봐야 알 수 있었습니다. 이제 앱을 켜면 새 버전을
확인하고, 있을 때만 타이틀바에 버튼이 나타납니다. 없으면 아무것도 보이지 않습니다.

- **Windows** — 배경으로 받아 두고, 「재시작하고 설치」를 누르면 재시작하며 설치까지 끝냅니다
- **macOS** — 새 버전을 알리고 dmg 를 받아 Finder 에서 열어줍니다. 드래그는 직접 하셔야 합니다

macOS 가 반쪽인 건 서명 때문입니다. 자동 설치를 담당하는 Squirrel.Mac 은 새 앱 번들의 서명이
지금 실행 중인 앱과 같은 주체인지 검사하는데, 이 앱은 ad-hoc 서명이라 통과하지 못합니다.
다운로드는 끝나고 재시작하면 예전 버전 그대로인 — 원인을 찾기 어려운 방식으로 실패해서,
아예 그 경로를 타지 않도록 했습니다. Apple Developer ID 서명을 붙이면 Windows 와 같아집니다.

업데이트 확인이 실패해도(오프라인·사내망 차단) 앱 사용에는 영향이 없습니다. 조용히 넘어갑니다.

---

외부 디자인 리뷰(v2.0.3 대상, 24건)를 반영했습니다. 대부분은 "규칙은 옳은데 값이 어긋난"
경우였고, 색을 더하는 대신 덜어내는 방향으로 고쳤습니다.

### 수정 — 다크 화면에서 카드가 카드로 안 보이던 문제

캔버스와 패널의 밝기 차이가 1.09:1 이라 영역 경계가 사실상 없었습니다. 어두운 면끼리는
밝기를 벌려도 대비가 안 나오기 때문에, 위계를 **테두리·그림자·표식**으로 옮겼습니다.
테두리를 눈에 보이는 밝기로 올리고, 모달·팝오버는 그림자와 밝은 테두리로 띄우고,
선택된 항목은 좌측 3px 레일로 표시합니다.

호버·부상·선택이 모두 같은 회색 한 톤이던 것(호버와 부상은 값이 아예 같았습니다)도
서로 다른 세 값으로 갈랐습니다.

### 수정 — 작은 회색 글씨가 안 읽히던 문제

캡션·placeholder·필드 라벨에 쓰이던 회색이 다크에서 3.4:1 로 접근성 기준(4.5:1)에
못 미쳤습니다. 색을 밝히고, **글자 크기 하한을 9px 에서 11px 로** 올렸습니다.
9~10px 로 찍히던 곳 357군데가 함께 올라갑니다.

### 수정 — 키보드 포커스가 어디 있는지 안 보이던 문제

포커스 링이 반투명이라 어두운 면 위에서 1.6:1 밖에 안 나왔습니다. 불투명한 2중 링으로
바꿔 13.7:1 을 확보했습니다. AI 생성 버튼은 포커스 표시를 아예 끄고 있어서 —
각 화면에서 가장 중요한 버튼인데 — 그 예외를 없앴습니다.

### 개선 — AI 브리핑

섹션 6개가 전부 다른 색 카드였습니다. 색이 여섯이면 무엇이 급한지 알 수 없어서
**카드 배경·테두리를 무채색으로 통일하고 색은 아이콘에만** 남겼습니다. 「긴급」은 좌측
붉은 레일을 하나 더 가져서, 여섯 아이콘이 유색이어도 심각도가 묻히지 않습니다.
「오늘 집중」 섹션만 배경이 비어 보이던 것도 이 과정에서 사라졌습니다.

- 본문 폭에 상한을 두고, 남는 자리에 섹션 목차를 넣었습니다
- 「착수 필요」의 경과 일수를 칩으로 꺼내 60일↑ 빨강 / 30일↑ 노랑으로 나눴습니다
- 삭제 버튼에 「이 브리핑 삭제」라고 이름을 붙였습니다 (히스토리 행 삭제와 같은 아이콘이었습니다)

### 개선 — 대시보드

- 지표 5개를 **조치가 필요한 3개**로 줄이고 전체·완료는 진행률 한 줄로 내렸습니다
- 「오늘 마감 0」이 좋은 신호인지 동기화 실패인지 구분되게 해석 문구를 붙였습니다
- **행을 누르면 앱 안에서 상세가 열립니다.** 두레이 원본은 행 끝 ↗ 로 분리했습니다
- 배지 숫자와 실제 목록 줄 수가 어긋나던 것을 맞췄습니다 (종료된 건 양쪽 모두 제외)
- 본문 입력칸의 예시 마크다운을 안내 한 줄로 바꾸고, 골격은 「템플릿 › 기본 골격」이 실제로 넣습니다
- 「두레이에 생성」이 완료 상태와 같은 초록이던 것을 주 버튼 색으로 바꿨습니다

### 수정 — 공유 목록에서 토큰이 그대로 보이던 문제

MCP 공유 탭의 카드가 실행 인자를 통째로 찍고 있어서 `Authorization: Bearer …` 가 화면에
남았습니다. 로컬 카드와 공유 카드가 이제 같은 컴포넌트로 인자를 그립니다 — 한쪽만 고치면
다른 쪽으로 다시 샙니다.

### 개선 — 스킬 · MCP 화면의 말과 모양을 맞췄습니다

네 화면(스킬 로컬·공유, MCP 로컬·공유)이 같은 개념을 각각 다르게 부르고 있었습니다.

- 공유 목록에서 가져오는 동작을 **「내려받기」로 통일**했습니다 (MCP 는 「적용」이었습니다)
- 로컬 카드의 액션 배치를 맞췄습니다 — 편집·공유만 아이콘으로 두고 **삭제는 ⋯ 메뉴 안으로**.
  스킬 카드는 삭제가 편집 옆에 무채색 아이콘으로 노출돼 있었습니다
- 인자 개수 표기가 `인자 3개` / `3개 인자` 로 갈리던 것을 맞췄습니다
- MCP 공유 탭이 **로컬 서버 개수**를 보여주던 것을 고치고, 카운트 표기를 스킬과 같은 칩으로 바꿨습니다
- 날짜를 전부 상대 시간으로 통일했습니다 (마우스를 올리면 정확한 시각이 나옵니다)
- 아이콘 버튼 크기를 26px 에서 32px 로 올렸습니다
- 비활성 상태를 빨강으로 칠하던 것을 무채색으로 바꿨습니다 (비활성은 오류가 아닙니다)

### 수정 — 파랑이어야 할 것들이 다크에서 회색으로 나오던 문제

`clauday-blue` 는 v2.0 에서 **다크일 때 회색으로 중성화되는 크롬 토큰**이 됐는데, 이름이
그대로여서 "파랑"인 줄 알고 쓴 자리가 많았습니다. 캘린더의 오늘 날짜, 커뮤니티 아이콘,
저장·생성 버튼이 전부 회색 덩어리로 나오던 이유입니다. 85곳을 용도대로 갈랐습니다.

- **주 버튼 12곳** — v2.0 이 만든 밝은 주 버튼(`ds-btn primary`)으로. 검정 캔버스에서
  저장·생성이 유일하게 튀는 면이 됩니다
- **AI 액션 버튼 7곳** — 그라디언트를 손으로 그리고 있어서 다크에서 **주황→회색**이 됐습니다.
  공용 AI 버튼으로 옮겨 화면마다 다르던 크기도 함께 맞췄습니다
- 도메인 대표 아이콘 — 각 도메인색으로 (Claude 주황 / 두레이 파랑 / 터미널 초록)
- 체크박스·토글 활성, 진행률 바, 카운트 배지 — 각각 상태색·워크플로색·배지색으로
- 목록의 선택 표시 — 회색 면 + 밝은 레일로 (다크에서 회색 테두리라 선택이 안 보였습니다)
- 로딩 스피너·리사이즈 핸들 — 무채색 토큰으로 **이름만** 정정 (동작은 그대로)

입력칸 31곳의 포커스 표시도 통일했습니다. 각자 다른 색을 쓰던 것을 공용 규칙 하나로 모아,
어느 화면에서 탭을 눌러도 같은 모양의 링이 나옵니다.

이 함정이 반복되지 않도록 `clauday-blue` 사용을 막는 가드를 넣었습니다.

### 수정 — 사이드바를 좁히면 「프로젝트」 글자가 세로로 쪼개지던 문제

### 개선 — 테마 토글 라벨

다크 모드에서 「Light」라고 적혀 있어 현재 테마 이름처럼 읽혔습니다. 이제 현재 상태를
보여주고, 할 동작은 툴팁이 알립니다.

### 내부

- 디자인 규칙을 지키는 회귀 가드 3종 추가 — 대비 계산, 조판 하한, 원시 팔레트 사용
- `docs/design-system` 문서를 실제 v2.0.4 값으로 재작성 (v1 값에 멈춰 있었습니다)

## [2.0.3] - 2026-08-03

### 수정 — 업무 상세에서 첨부 이미지가 안 나오던 문제

두레이 첨부는 게시글 경로로만 받을 수 있는데, 작업 패널이 쓰는 상세 창에만 그 정보가
빠져 있었습니다. 두레이 뷰에서는 잘 나오고 여기서만 "두레이에서 보기" 링크로 떨어지던
이유입니다. 본문과 댓글의 이미지 모두 이제 제대로 나옵니다.

### 수정 — 새 터미널 탭에 바로 타이핑이 안 되던 문제

⌘T 로 탭을 열면 한 번 클릭해야 입력이 됐습니다. 터미널 화면이 아직 화면에 붙기 전에 포커스를
잡으려 해서 그 시도가 무시된 것입니다. 이제 붙은 직후 다시 잡습니다. 화면을 분할하거나 닫을
때 포커스가 사라지던 것도 함께 고쳤습니다.

### 수정 — 브랜치 이름 규칙의 `{subject}` · `{prefix}` 가 빈칸이 되던 문제

설정에 넣을 수 있는데 결과에는 안 나왔습니다. 업무를 터미널에 놓아 시작하는 경로에서만
이 값들을 넘기지 않고 있었습니다. 설정 화면의 미리보기도 실제와 같은 값으로 보여줍니다 —
미리보기가 결과와 다르면 설정을 믿을 수 없습니다.

### 수정 — 브랜치 이름에서 한글이 사라지던 문제

`feature/MIS-경영정보서비스/1705-qa-{taskNumber}` 를 넣으면 `feature/MIS/1705-qa-6793` 이
됐습니다. git 은 한글 브랜치 이름을 허용하는데 앱이 영숫자만 남기고 있었습니다. 이제 적은
대로 만들어집니다. 공백이나 `~^:?*` 처럼 git 이 못 쓰는 문자만 걸러냅니다.

### 수정 — 실시간 모드 재연결이 바로 안 붙던 문제

재연결을 눌러도 한참 `CONNECTING` 에 머물렀습니다. 연결을 닫자마자 새로 붙는 바람에 서버가
아직 옛 세션을 들고 있어 거절하고, 그 뒤 15초를 더 기다리게 되는 구조였습니다. 이제 연결이
정리될 때까지 잠깐 기다렸다가 붙습니다.

### 수정 — 화면이 좁을 때 탭바의 버튼이 사라지던 문제

탭이 늘어나면 오른쪽의 분할 · 작업 패널 버튼이 스크롤 밖으로 밀려나 누를 수 없었습니다.
이제 탭 목록만 스크롤되고 버튼은 제자리에 남습니다.

### 추가 — 업무를 상태 · 태그 · 단계로 좁혀 봅니다

작업 패널 검색창 옆 버튼에서 고릅니다. 같은 항목에서 여러 개를 고르면 그중 하나라도 맞는
업무가, 항목을 넘나들면 모두 맞는 업무가 남습니다. 걸어둔 조건은 검색창 아래 칩으로 보이고
눌러서 뺍니다.

### 추가 — 업무 본문을 상세 창에서 바로 고칩니다

✏ 를 누르면 그 자리에서 고쳐 두레이에 저장합니다. 읽어온 형식(HTML · 마크다운)을 그대로
유지하므로 두레이 웹에서 만든 표나 체크박스가 깨지지 않습니다. 편집 중 Esc 는 창이 아니라
편집만 닫습니다.

### 추가 — 워크트리 폴더를 IDE 에서 바로 엽니다

작업 패널 브랜치 탭의 워크트리 행과 터미널 탭바에 열기 버튼이 생겼습니다. 설치된 것만
보여주고(IntelliJ · WebStorm · PyCharm · VS Code · Cursor), 여러 개면 골라서 엽니다.
JetBrains Toolbox 로 설치해 `idea` 명령이 없어도 찾아냅니다.

### 추가 — 세션 보관 기간을 앱에서 정합니다

즐겨찾기해둔 Claude 세션이 목록에서 사라지는 원인은 Claude Code 가 오래된 세션 기록을
지우기 때문이었습니다(기본 30일). Claude 채팅의 세션 목록 헤더 ⚙ 에서 이 기간을 늘립니다.
`~/.claude/settings.json` 을 고치는 것이라 터미널에서 쓰는 claude 에도 그대로 적용됩니다.

### 추가 — 설정이 덜 됐으면 무엇을 해야 하는지 알려줍니다

업무 목록이 비어 있을 때 이유를 갈라 보여줍니다 — 볼 프로젝트를 안 골랐는지, 저장소를 아직
등록 안 했는지, 이 프로젝트에 쓸 저장소를 안 정했는지. 각 안내에서 설정 화면으로 바로
갑니다. 업무는 떠도 저장소를 안 정했으면 목록 위에서 미리 알립니다.

### 변경 — 설정의 템플릿 토큰을 눌러서 복사합니다

브랜치 이름 · 첫 지시 문구 아래 `{taskNumber}` 같은 토큰을 누르면 클립보드에 담깁니다.
중괄호까지 손으로 옮겨 적다 오타를 내면 그 자리가 조용히 빈칸이 됐습니다.
