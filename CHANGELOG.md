...
### 수정 — 워크트리에서 시작한 업무는 이어갈 수 없었다

업무를 드롭하면 브랜치 워크트리로 옮겨가는데, 세션 연결은 **폴더 경로**로 기억합니다.
워크트리 경로는 저장소 경로와 달라서, 저장소 경로로만 찾던 "이어가기" 는 영영 안 붙고
resume 도 되지 않았습니다. 저장소에서 그대로 돈 경우만 우연히 경로가 같아 동작했습니다.

- 연결에 **저장소 경로를 함께** 남겨 저장소 단위로 세션을 찾습니다. 이전에 만들어진 연결도
  워크트리 폴더 이름 규칙으로 되짚으므로 다시 드롭할 필요는 없습니다.
- 이어갈 때는 **세션이 만들어진 폴더**로 갑니다 — 세션은 그 폴더에서만 resume 됩니다.
  저장소 선택창도 실제로 들어갈 워크트리 경로를 보여줍니다.

### 수정 — 연결 배지가 2분씩 늦게 뜨던 문제

세션이 생기길 기다리는 확인이 렌더러에서 8초 → 20초 → 45초 세 번뿐이라, 늦게 뜨거나
아예 안 뜨는 경우가 있었습니다. 이제 **2초 간격으로 3분** 동안 지켜보고, 연결되는 즉시
배지를 붙입니다. 확인은 앱 본체에서 돌기 때문에 탭을 옮기거나 다른 화면으로 가도 이어집니다.

### 수정 — 브랜치 이름이 `feature/task-1234` 로 만들어지던 문제

프로젝트 코드를 캐시에서만 읽어, 프로젝트 목록을 한 번도 부르지 않은 화면(터미널 업무 목록)
에서는 항상 비어 있었습니다. 이제 필요하면 받아옵니다 — `feature/neon-6793` 처럼 제대로
나옵니다. 잘못된 이름으로 이미 만들어진 워크트리는 ⚙ 설정 → **워크트리 정리** 에서 지웁니다.

### 변경 — 워크트리를 못 만들면 알려준다

git 은 같은 브랜치를 두 곳에 체크아웃하지 못합니다. 저장소가 이미 그 브랜치에 있으면
워크트리 없이 저장소에서 시작하는데, 지금까지는 조용히 그렇게 했습니다. 이제 알립니다.
...
- **웹폰트 경쟁**: 기본 글꼴(JetBrains Mono)을 Google Fonts 에서 받아오는데, 늦게 도착하면
  xterm 은 이미 폴백 글꼴로 글리프를 캐시한 뒤였습니다. 이제 폰트가 준비되면 캐시를 비우고
  다시 맞춥니다. 기본 글꼴도 **시스템 고정폭**으로 바꿨습니다 — 내려받지 않아 가장 또렷합니다.
- **화면 배율 변경**: 모니터를 옮기거나 배율이 바뀌어도 글리프 캐시가 옛 배율로 남았습니다.
  이제 배율이 바뀌면 캐시를 비웁니다.

### 추가 — 터미널 글꼴 설정

- ⚙ 설정 → 외관 → **터미널 글꼴**: 글꼴(시스템 고정폭 · JetBrains Mono · D2Coding ·
  Sarasa Mono K · Fira Code · Cascadia Mono) · 크기 · 굵기 · 줄 간격.
- 각 글꼴은 실제로 그 글꼴로 그린 미리보기(`git commit -m "한글 Ag 0O1l"`)로 보여줍니다.
- 바꾸면 열려 있는 터미널에 바로 반영됩니다. 한글 폴백은 어떤 선택에도 항상 붙습니다.

### 변경 — 업무↔세션 연결은 자동으로만

- 카드의 **연결 해제 버튼을 없앴습니다.** 드롭하면 알아서 매핑되고, 그 관계를 손으로 끊을 일은
  드물었습니다.
- **연결이 안 붙던 문제**를 고쳤습니다: 세션 파일이 늦게 생기면 한 번 확인하고 포기했습니다.
  이제 몇 번 더 확인합니다(8초 → 20초 → 45초). 배지 이름도 워크트리에서 `저장소 · 브랜치` 로
  제대로 나옵니다 — 전에는 등록된 저장소 경로와 달라 이름을 못 찾았습니다.
