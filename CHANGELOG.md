...
### 변경 — 변경사항에서 커밋할 파일을 체크로 고릅니다

- 각 파일 앞의 **체크박스가 곧 커밋 대상**입니다(git 의 스테이징을 그대로 씁니다).
  **추적되지 않은 새 파일도 체크하면 그대로 커밋에 들어갑니다.**
- 섹션 헤더 체크박스로 한 번에 넣고 뺍니다. 일부만 골랐으면 중간 상태로 표시합니다.
- 같은 일을 하던 hover 버튼(`전체 올리기`/`전체 내리기`)은 없앴습니다 — 둘이 나란히 있으면
  뭐가 다른지 알 수 없습니다.
