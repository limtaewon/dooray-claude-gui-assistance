...
### 새 기능 — 워크스페이스 (터미널 뷰에 통합, 사이드바 ⌨️)

터미널이 곧 작업 화면입니다. 평소처럼 터미널을 쓰다가 우측 두레이 패널에서 업무를 꺼내 쓰는 구조로 통합했습니다.

- **우측 두레이 패널 (⌘⇧T)** — 탭바의 `📋 태스크` 로 열립니다. 내 업무 목록이 `프로젝트/번호 → 제목 → 상태` 카드로 보이고, 워크스페이스가 있는 업무에는 브랜치와 진행 상태 점이 함께 표시됩니다.
- **업무 카드 클릭 → 상세 오버레이** — 본문·댓글을 넓게 읽고, 그 자리에서 `워크스페이스 시작`·`업무 번호 복사`·`프롬프트 복사`·댓글 작성까지 합니다. 이미 워크스페이스가 있으면 클릭만으로 해당 터미널 탭으로 이동합니다.
- **워크스페이스 시작** — 워크트리 + 브랜치를 만들고 그 폴더에서 claude 를 실행한 뒤, **새 터미널 탭으로 열립니다.** 업무마다 워크트리가 분리되므로 여러 브랜치를 탭으로 나란히 진행할 수 있습니다. 두레이 상태 '진행중' 전환과 브랜치명 댓글도 옵션으로 함께 처리됩니다.
- **카드를 터미널에 끌어다 놓기** — 워크트리 없이 매핑된 저장소로 `cd` 한 뒤 claude 를 실행하는 가벼운 경로입니다. 한 번 작업한 업무는 `세션 연결됨` 배지가 붙고 다시 드롭하면 `claude --resume` 으로 이어갑니다.
- **프롬프트를 비우면** 자동 입력 없이 claude 만 실행됩니다 — 첫 지시는 터미널에서 직접.
- ⚙ 설정 → **워크스페이스** 에서 저장소를 등록하고(저장소별 기본 base·프리픽스), 브랜치 이름 템플릿(라이브 미리보기)과 작업 시작 기본값을 조정합니다.

### 변경 — 다크 테마 색감 개편

- 캔버스를 **거의 순검정**으로 낮추고 표면 간 대비를 줄였습니다. 컬러는 상태 칩과 AI 액션에만 남기고 나머지는 회색조 위계로만 표현합니다.
- 주 버튼이 그라디언트에서 **밝은 면 + 어두운 글자**로 바뀌었습니다(검정 캔버스에서 유일하게 튀는 요소).
- 카드·모달의 여백과 라운드를 키우고 경계선을 거의 보이지 않는 수준으로 낮췄습니다.
