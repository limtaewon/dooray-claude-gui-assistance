...
늘어서 이름을 '두레이 업무 패널' 에서 **사이드 패널** 로 바꿨습니다. **왼쪽 모서리를 끌어 폭을
...
- **커밋 검색** — 메시지 / 작성자 / 파일 경로 / 코드 내용(그 문자열이 추가·삭제된 커밋) 중에서 고릅니다.
  전부 있는 그대로 찾으므로 `feat(git):` 같은 문자열도 그대로 검색됩니다. 검색 중에는 그래프가 접힙니다
  — 결과가 히스토리의 일부라 부모 커밋이 없어 선을 이을 수 없습니다.
