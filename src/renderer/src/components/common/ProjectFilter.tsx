                  ...
                  <div key={p.id} className="flex items-center group">
                    <button onClick={() => toggle(p.id)}
                      ...
                      {isCustom ? (
                        ...
                        <FolderOpen size={11} className={`flex-shrink-0 ${checked ? 'text-blue-400' : 'text-gray-500'}`} />
                      )}
                      <span className={`text-[11px] truncate min-w-0 ${checked ? 'text-gray-100' : 'text-gray-400'}`}>{p.code}</span>
                    </button>
                    {isCustom && (
