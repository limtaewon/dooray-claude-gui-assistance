...
function ProjectFilter({ settingsKey = 'pinnedProjects', useWikiDomains = false, onChanged }: ProjectFilterProps): JSX.Element {
  ...
  const handleAddProject = async (): Promise<void> => {
    ...
    try {
      ...
      const nextCustom = [...customProjects, { id: project.id, code: project.code }]
  ...
  const filtered = mergedProjects.filter(p => !searchQuery || p.code.toLowerCase().includes(searchQuery.toLowerCase()))
    ...
    <div className="relative">
      ...
      {open && (
        <>
          ...
          <div className="absolute left-0 top-full mt-1 w-72 bg-[#151c2c] border border-[#2d3a52] rounded-xl shadow-2xl z-40 overflow-hidden">
                ...
                return (
                  <div key={p.id} className="flex items-center group">
                    <button onClick={() => toggle(p.id)}
                      className="flex-1 flex items-center gap-2 px-3 py-1.5 hover:bg-[#1e2840] transition-colors text-left">
                      ...
                      {isCustom ? (
                        <Link size={11} className={checked ? 'text-amber-400' : 'text-amber-600'} />
                      ) : (
            ...
            <div className="border-t border-[#2d3a52]">
              {showAddForm ? (
                <div className="px-2 py-2 space-y-1.5">
                  <div className="flex gap-1">
                    <input
                      ...
                      className="flex-1 px-2 py-1 bg-[#0d1321] border border-[#2d3a52] rounded text-[11px] text-gray-100 placeholder-gray-500 focus:outline-none focus:border-blue-500"
                      autoFocus
                    />
                    <button onClick={handleAddProject} disabled={adding || !addInput.trim()}
                      className="px-2 py-1 bg-blue-600 hover:bg-blue-500 disabled:bg-gray-700 disabled:text-gray-500 rounded text-[10px] text-white transition-colors">
                      {adding ? '...' : '추가'}
                    </button>
                  </div>
                ...
                <button onClick={() => setShowAddForm(true)}
                  className="w-full flex items-center gap-1.5 px-3 py-1.5 text-[10px] text-gray-500 hover:text-gray-300 hover:bg-[#1e2840] transition-colors">
                  <Plus size={10} />
                  프로젝트 수동 추가 (ID/URL)
                </button>
              )}
