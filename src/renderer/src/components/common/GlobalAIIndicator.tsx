...
function GlobalAIIndicator(): JSX.Element | null {
  ...
  return (
    <div className="flex items-center gap-1.5 px-3 py-1 rounded-full bg-bg-surface-raised border border-bg-border-light">
      <div className="relative flex items-center justify-center">
        <Sparkles size={11} className="text-clauday-orange" />
        <Loader2 size={14} className="absolute text-text-tertiary animate-spin" />
      </div>
      <span className="text-[calc(11px_*_var(--app-font-scale,1))] text-text-primary font-medium truncate max-w-[200px]">
        {active.message}
      </span>
      <span className="text-[calc(11px_*_var(--app-font-scale,1))] text-text-tertiary font-mono">{timeStr}</span>
    </div>
  )
}
