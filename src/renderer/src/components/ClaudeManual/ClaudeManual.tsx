...
- **작업 패널 (⌘⇧T)** — 탭바 오른쪽 버튼으로 열립니다. \`업무\` 탭에서 업무 카드를 터미널 화면에 **끌어다 놓으면** 매핑된 저장소로 \`cd\` 하고 claude 가 실행되면서 업무 내용을 첫 지시로 전달합니다. 한 번 작업한 업무에는 \`세션 연결됨\` 배지가 붙고, 다음부터 드롭하면 \`claude --resume\` 으로 이전 대화를 이어갑니다(프롬프트 재입력 없음). 연결을 끊으려면 배지 옆 아이콘을 누르세요. 저장소는 ⚙ 설정 → 워크스페이스에서 등록합니다.
