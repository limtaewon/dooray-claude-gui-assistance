...
const SECTIONS: Section[] = [
  {
    id: 'terminal-scm', icon: GitBranch, title: '터미널 작업 패널',
    content: `# 작업 패널 — 업무 · 변경사항 · 히스토리 · 브랜치

기존 **브랜치 작업** 메뉴가 없어지고 그 기능이 **터미널 작업 패널**로 들어왔습니다.
터미널을 보면서 같은 화면에서 커밋까지 끝내기 위해서입니다.

패널은 \`⌘⇧T\` 로 열고 닫으며, 상단 아이콘 4개로 탭을 바꿉니다.
**왼쪽 모서리를 끌어 폭을 조절**할 수 있고(더블클릭하면 기본 폭으로), 폭은 저장됩니다.

| 탭 | 내용 |
|----|------|
| 📋 **업무** | 고정한 프로젝트의 내 두레이 업무. 카드를 터미널로 끌어다 놓으면 그 폴더에서 시작 |
| 🔀 **변경사항** | 파일 목록 · 스테이징 · 커밋 · 풀/푸시 |
| 🕘 **히스토리** | 색상 레인 커밋 그래프 · 커밋 검색 |
| 🌿 **브랜치** | 브랜치 전환/생성, 워크트리, 스태시 |

## 어떤 저장소를 보나

기본은 **지금 보고 있는 터미널의 폴더**가 속한 저장소를 따라갑니다. 터미널 탭을 바꾸면 패널도
따라 바뀝니다.

패널 위쪽의 **저장소 이름을 누르면** 목록이 열립니다. 여기서 최근에 본 저장소를 고르거나
\`폴더 열기…\` 로 아무 저장소나 지정하면 그 저장소로 **고정**됩니다(터미널을 옮겨도 안 바뀝니다).
\`터미널 따라가기\` 를 고르면 다시 자동으로 돌아갑니다.

> 고정이 필요한 경우: Windows 는 셸에서 \`cd\` 한 것을 추적할 수 없고, 터미널을 안 연 상태에서도
> 저장소를 봐야 할 때가 있으며, A 저장소에서 작업하며 B 를 들여다보고 싶을 때가 있습니다.

## 변경사항

- 파일 행에 마우스를 올리면 **올리기(+) · 내리기(−) · 되돌리기(↩)** 가 나옵니다.
- 섹션 헤더의 **+ / −** 로 그 섹션 전체를 한 번에 처리합니다.
- 파일을 클릭하면 **좌우 비교 diff** 가 전체 화면으로 열립니다 (\`Esc\` 로 닫기).
- **직전 커밋 수정** 을 체크하면 이전 메시지를 자동으로 불러옵니다 (\`--amend\`).
- 머지/리베이스 충돌 중에는 배너가 뜨고 **중단** 버튼이 나옵니다. 충돌이 남아 있으면 커밋이 막힙니다.

## 히스토리

- 커밋을 펼치면 그 커밋의 변경 파일이 나오고, 파일을 클릭하면 그 시점 diff 탭이 열립니다.
- **브랜치 칩**으로 조회 범위를 정합니다. 전체를 봐도 브랜치·원격·태그만 보므로 스태시는 섞이지 않습니다.
- **더 보기** 로 50개씩 이어서 불러옵니다.

### 커밋 검색 · 필터

IntelliJ 의 git log 툴바와 같은 방식입니다 — **검색어 한 칸 + 조건 칩들**이고, 조건들은
서로 배타적이지 않고 **함께** 걸립니다. "이 경로에서 저 사람이 지난주에 만진 커밋" 같은 조합이 됩니다.

**검색창** — 커밋 메시지를 찾습니다. 7자 이상 hex 를 넣으면 **해시로 알아듣고 그 커밋 하나만**
보여줍니다. 오른쪽 두 토글:

| 토글 | 뜻 |
|------|-----|
| \`.*\` | 검색어를 정규식으로 해석 (끄면 있는 그대로 — \`feat(git):\` 도 그대로 찾습니다) |
| \`Aa\` | 대소문자 구분 (기본은 무시) |
...
| 칩 | 거르는 기준 |
|----|-------------|
| **브랜치** | 현재 브랜치 / 모든 브랜치 / 특정 브랜치 |
| **작성자** | 저장소의 실제 작성자 목록에서 고릅니다(자주 커밋한 순). 검색으로 좁힐 수 있습니다 |
| **기간** | 오늘·최근 7일·30일·3개월 프리셋 또는 직접 입력(\`2026-07-01\`) |
| **경로** | 그 경로를 건드린 커밋 |
| **코드** | 그 문자열이 **추가되거나 삭제된** 커밋 |

조건이 하나라도 걸리면 오른쪽에 \`지우기\` 가 나옵니다.
검색 중에는 그래프가 접힙니다 — 결과가 히스토리의 일부라 부모 커밋이 목록에 없어 선을 이을 수 없습니다.

## 브랜치

- 워크트리를 만들면 **새 터미널 탭**이 그 폴더로 열립니다 — 브랜치별 병렬 작업의 출발점입니다.
- 목록의 워크트리를 클릭해도 새 터미널이 열립니다.
- 스태시는 추적되지 않은 파일까지 함께 보관합니다.

## 갱신 시점

폴링하지 않습니다. **진입할 때 · 여기서 무언가를 바꿨을 때 · 새로고침 버튼 · 다른 앱에서 돌아왔을 때**
다시 읽습니다. 터미널에서 직접 \`git commit\` 을 했다면 새로고침을 누르거나 창을 한 번 벗어났다 오세요.
`
  },
  {
...
| >_ | **터미널** | 내장 터미널 + 작업 패널(업무 · 변경사항 · 히스토리 · 브랜치) |
...
⚙ 설정 → **단축키** 에서 앱의 모든 단축키를 확인하고 원하는 조합으로 바꿀 수 있습니다.

- 그룹별 전체 목록 + 액션명·키 조합 검색, \`변경됨\` / \`충돌\` 필터.
- 키 조합을 클릭하면 캡처 모드가 되고, 원하는 조합을 누르면 즉시 저장됩니다(Esc 취소). 행 오른쪽 ↺ 로 기본값 복원.
- 같은 범위에서 이미 쓰는 조합이면 어떤 기능과 겹치는지 경고합니다.
- 한글 입력 상태에서도 물리 키 기준으로 인식하며, mac 에서 정한 조합은 Windows 에서 \`⌘\`→\`Ctrl\` 로 자동 대응됩니다.
- 🔒 표시는 셸 제어문자(줄 이동·복사 등)나 시스템 메뉴가 소유해 변경할 수 없는 항목입니다.

---

## 🗂 워크스페이스 (v2.0)

두레이 업무에서 곧바로 브랜치·워크트리·claude 터미널까지 한 번에 여는 화면입니다. 사이드바의 **워크스페이스** 로 들어갑니다.

- **먼저 저장소를 등록하세요** — ⚙ 설정 → 워크스페이스 → \`폴더 추가\` 로 git 저장소를 등록합니다. 저장소별 기본 base 브랜치와 브랜치 프리픽스를 지정할 수 있습니다.
- **[▶ 작업 시작] 원클릭** — 왼쪽에서 업무를 고르고 버튼을 누르면 끝입니다. 버튼 아래에 \`저장소 · base · 브랜치 · 에이전트 1\` 요약이 미리 보이므로 누르기 전에 무엇이 만들어질지 알 수 있습니다. 워크트리와 브랜치가 생성되고 그 폴더에서 claude 가 실행됩니다.
- **설정을 바꿔 시작하려면** — 버튼 오른쪽 \`▾\` 또는 \`⌥클릭\` 으로 모달을 엽니다. 저장소·base·브랜치 이름·프롬프트·권한 자동 승인·두레이 상태 전환을 그 자리에서 조정할 수 있습니다.
- **프롬프트를 비우면** 자동 입력 없이 claude 만 실행됩니다 — 첫 지시는 터미널에서 직접 하세요. \`▾ → 프롬프트 없이 터미널만 열기\` 도 같은 동작입니다.
- **여러 업무를 동시에** — 업무마다 워크트리가 분리되므로 여러 브랜치를 나란히 굴릴 수 있습니다. 왼쪽 목록의 브랜치 배지와 상태 점(파랑=작업 중, 주황=응답 대기)으로 진행 상황이 보이고, 클릭으로 전환합니다.
- **업무 본문을 보면서 작업** — 오른쪽 패널의 \`업무 상세\` 탭에서 본문·댓글을 읽고 두레이 상태도 바꿀 수 있습니다. \`변경사항\` 탭은 지금 워크트리의 미커밋 diff 를 보여줍니다.
- **재연결 / 채택 / 정리** — 앱을 재시작하면 터미널 세션이 끊깁니다. \`재연결\` 을 누르면 이전 claude 세션을 \`--resume\` 으로 이어서 엽니다. 작업이 끝나면 \`채택\` 으로 표시하고, \`정리\` 로 워크트리를 제거합니다(미커밋 변경이 있으면 함께 사라지므로 확인을 받습니다).

---

## 🖥 터미널 (v2.0)

- **세션 종료 오버레이** — 셸이 끝나면(\`exit\`) pane 에 종료 배지가 뜨고 그 pane 의 입력이 막힙니다. 스크롤·선택·복사는 계속 가능하며, 로그는 직접 닫기 전까지 그대로 남습니다. 탭을 직접 닫거나 앱을 종료할 때는 뜨지 않습니다.
- **검색 고도화 (⌘F)** — 매치 카운트(\`3/47\`)가 표시되고, \`Aa\`(대소문자) · \`.*\`(정규식) · \\b(단어 단위) 토글이 있습니다. 잘못된 정규식을 입력해도 터미널은 죽지 않고 오류만 표시됩니다. 우측 오버뷰 룰러에 매치 위치가 마커로 표시됩니다.
- **탭 드래그 순서 변경** — 탭을 12px 이상 끌면 순서를 바꿀 수 있습니다(더블클릭은 여전히 이름 변경). 탭을 닫으면 최근 사용한 탭이 우선 활성화됩니다(MRU, 없으면 오른쪽 → 왼쪽 이웃). 순서는 재시작 후에도 이름 변경과 비슷한 수준으로 유지됩니다.
- **Split pane (탭 안에서 화면 나누기)** — \`⌘D\`로 오른쪽, \`⌘⇧D\`로 아래로 나뉩니다. 분할은 항상 **새 셸 세션**을 엽니다(현재 pane 의 작업 폴더를 이어받음). 한 탭 안에서 3분할 이상, 중첩(행 안에 열)도 가능합니다. \`⌥⌘←↑↓→\`로 pane 사이를 이동하고, \`⌘W\`는 포커스된 pane을 먼저 닫고(마지막 pane 이면 탭 자체를 닫습니다). 터미널에 **EOF**를 보내려면 (\`⌘D\`가 아니라) \`Ctrl+D\`를 쓰세요.
- **경계 드래그로 크기 조절** — pane 사이 경계를 드래그하면 비율이 바뀌고, 더블클릭하면 50/50 으로 되돌아갑니다. 드래그 중에는 화면만 움직이고 실제 터미널 크기는 손을 놓는 순간 한 번만 반영됩니다(vim 등 화면 전체를 그리는 프로그램이 드래그 중 매 프레임 다시 그리지 않도록).
- 탭 라벨에 분할된 pane 수(\`⫿2\`)가 배지로 표시됩니다.
- **화면까지 통째로 복원** — 탭 구성·분할·**화면 내용(스크롤백·커서·색)** 이 저장되어 재시작해도 그대로 돌아옵니다(탭 최대 20개). 창을 닫고 한참 뒤에 완전히 종료해도(예: macOS 독에 남겨뒀다가 나중에 \`⌘Q\`) 스크롤백이 사라지지 않습니다.
- **작업 패널 (⌘⇧T)** — 탭바 오른쪽 버튼으로 열립니다. \`업무\` 탭에서 업무 카드를 터미널 화면에 **끌어다 놓으면** 그 자리에서 claude 가 실행되고 업무 내용이 첫 지시로 전달됩니다.
  - **항상 그 업무 전용 워크트리에서 시작합니다.** 브랜치 이름은 프로젝트 규칙(\`feature/{projectCode}-{taskNumber}\`)을 따르고, 폴더는 저장소 옆 \`.<저장소>-worktrees/<브랜치>\` 에 생깁니다. 다른 업무가 저장소를 점유하고 있어도 막히지 않아 **여러 업무를 동시에** 진행할 수 있습니다. 커밋·푸시는 그 워크트리에서 그대로 하면 규칙대로 만들어진 브랜치가 올라갑니다(첫 푸시는 자동으로 \`-u\`).
    - 그 브랜치가 **이미 어딘가에 체크아웃돼 있으면 그 폴더를 그대로 씁니다** — 저장소 폴더에서 이미 그 업무를 하고 있었다면 거기서 이어집니다.
    - 워크트리를 만들지 못하면(서브모듈 등) 알려주고 저장소 폴더에서 그대로 시작합니다.
    - 새 브랜치는 저장소의 \`기본 base\`(⚙ 설정 → 워크스페이스 → 저장소) → \`origin/HEAD\` 순으로 갈라냅니다. 둘 다 없을 때만 현재 브랜치를 기준으로 씁니다.
    - **정리**: 작업 패널 → 브랜치 탭 → 워크트리 옆 \`정리…\` 에서 오래 안 쓴 순으로 보고 여러 개를 골라 지웁니다. 용량·마지막 사용·커밋 안 된 변경이 함께 보이고, **브랜치와 커밋은 지워지지 않아** 같은 업무를 다시 시작하면 이어집니다.
  - **어디 저장소에서 할지는 규칙으로 정해집니다** (⚙ 설정 → 워크스페이스 → 프로젝트별 규칙에서 저장소를 지정해 두세요).
    - 지금 터미널이 **그 프로젝트의 저장소 안**이면 이동 없이 그 자리에서 시작합니다. 터미널을 그 폴더로 옮겨둔 것 자체가 선택이라 덮지 않습니다.
    - 매핑되지 않은 곳에 놓았고 저장소가 **여러 개**면 어디서 할지 묻는 창이 뜹니다. 고르면 그 폴더로 이동해 시작합니다.
    - 저장소가 **하나**뿐이면 묻지 않고 그리로 이동합니다. 지정한 저장소가 없으면 지금 자리에서 그대로 시작합니다.
    - 업무 상세의 **터미널에서 시작** 버튼도 같은 규칙을 따릅니다.
  - **claude 나 vim 이 돌고 있는 터미널에는 놓아도 안전합니다.** 실행 중이면 그 자리에 타이핑하지 않고 **새 탭**을 열어 거기서 시작합니다(진행 중인 대화가 오염되지 않습니다).
  - 시작하면 어디서 시작했는지 알림으로 알려줍니다 — 엉뚱한 폴더로 갔다면 바로 알 수 있습니다.
  - 그 폴더에서 이 업무로 쓰던 세션이 있으면 \`claude --resume\` 으로 이어갑니다(프롬프트 재입력 없음). 세션은 **(업무 × 폴더)** 로 기억하므로 저장소마다 따로 이어집니다.
  - 카드에는 세션이 있는 **저장소마다 배지**가 붙습니다. 누르면 그 세션이 새 터미널 탭에서 열립니다.
  - **첫 지시 문구는 설정에서 바꿉니다.** \`{title}\` \`{number}\` \`{project}\` \`{ref}\` \`{url}\` \`{body}\` 를 쓸 수 있고, 비우면 아무것도 보내지 않고 claude 만 띄웁니다.
  - 워크스페이스 뷰와의 차이: 드로어는 **워크트리를 만들지 않고** 저장소 폴더에서 바로 여는 가벼운 방식이고, 워크스페이스 뷰는 업무마다 브랜치·워크트리를 분리하는 방식입니다. 세션 연결은 두 흐름이 공유합니다.
- **diff 에서 변경점 이동** — diff 탭 오른쪽 위 \`∧\` \`∨\` 로 변경점을 순서대로 훑습니다. 지금 몇 번째인지(\`3/12\`)가 옆에 표시됩니다. 에디터에 포커스가 있으면 \`F7\`(다음) / \`Shift+F7\`(이전) 로도 이동합니다.
- **작업 패널 \`브랜치 변경\` 탭** — 이 브랜치가 기준(base)에서 갈라진 뒤 바꾼 파일을 모아 봅니다. \`변경사항\` 탭이 아직 커밋 안 한 것만 보여주는 것과 달리 **커밋한 것까지 합쳐서** 보여주므로, 업무 워크트리에서 "이 브랜치가 결국 뭘 바꾸나" 를 확인할 때 씁니다. 기준은 \`origin/HEAD\` → \`main\`/\`master\` 순으로 찾고, 파일을 누르면 기준 대비 diff 가 탭으로 열립니다. 새로 만든(git 이 아직 모르는) 파일은 \`변경사항\` 탭에 있습니다.
- **렌더러 전환 (WebGL/DOM)** — 탭바 우측의 \`렌더러: WebGL ▾\` 에서 전환할 수 있습니다(⚙ 설정 → 외관 & 동작 에도 동일 항목). 기본은 GPU 가속(WebGL)이고, 화면이 깨지거나 느리면 DOM 으로 바꾸세요. WebGL 이 자동으로 실패하면 라벨이 \`DOM (폴백)\`으로 바뀝니다.
- **경로 Cmd+클릭으로 열기 (개선)** — \`Cmd\`(mac)/\`Ctrl\`(win)+클릭으로 파일·폴더가 열립니다. **상대 경로**(\`src/main/index.ts\`), **공백 포함 경로**(\`Application Support\`), **확장자 없는 폴더**, \`Makefile\`/\`Dockerfile\` 같은 **무확장자 파일**, claude 출력처럼 **줄바꿈으로 잘린 긴 경로**까지 모두 인식합니다. \`파일.ts:120:8\` 형태는 줄 번호까지 툴팁에 표시됩니다. 실제로 존재하는 경로만 밑줄이 그어집니다. vim 등 마우스를 쓰는 프로그램 위에서 Cmd+클릭해도 이중으로 열리지 않습니다.
- **한글/이모지 표시 개선** — ZWJ 로 합쳐진 이모지(👨‍👩‍👧‍👦 등)의 폭이 정확히 계산되어 깨지지 않습니다.
...
|--------|------|------|
| \`⌘T\` | 새 터미널 탭 | 터미널 |
| \`⌘W\` | pane 닫기 → 마지막이면 탭 닫기 | 터미널 |
| \`⌘D\` | 오른쪽으로 분할(새 세션) | 터미널 |
| \`⌘⇧D\` | 아래로 분할(새 세션) | 터미널 |
| \`⌥⌘←↑↓→\` | 분할된 pane 사이 포커스 이동 | 터미널 |
| \`⌘1~9\` | 터미널 탭 전환 | 터미널 |
| \`⌘F\` | 터미널 검색 | 터미널 |
| \`⌘K\` | 커맨드 팔레트 | 전역 |
...
### Windows 참고
- 커맨드에 \`npx\`/\`uvx\` 같은 배치 런처를 입력하면, 저장 시 자동으로 \`cmd /c\` 로 감싸져 저장됩니다 (직접 \`cmd /c\` 를 입력할 필요 없음). 입력창 아래 안내 문구로 예고됩니다.
...
- (Windows) 위키 제목에 \`:\`, \`/\` 같은 Windows 금지문자가 있으면 내려받을 때 파일명이 자동으로 정리됩니다. 정리된 경우 실제 저장된 이름이 토스트로 안내됩니다.
