...
const SECTIONS: Section[] = [
  {
    id: 'terminal-scm', icon: GitBranch, title: '터미널 소스 제어',
    content: `# 터미널 우측 패널 — 업무 · 소스 제어

기존 **브랜치 작업** 메뉴가 없어지고 그 기능이 **터미널 우측 패널**로 들어왔습니다.
...
| 탭 | 내용 |
|----|------|
| 📋 **두레이 업무** | 고정한 프로젝트의 내 업무. 카드를 터미널로 끌어다 놓으면 그 폴더에서 시작 |
...
**지금 보고 있는 터미널의 폴더**가 속한 저장소를 따라갑니다. 탭을 바꾸면 패널도 따라 바뀝니다.
git 저장소가 아니면 각 탭이 무엇을 하는 곳인지 안내가 뜹니다.
...
| >_ | **터미널** | 내장 터미널 + 두레이 업무 · 소스 제어(변경사항·히스토리·브랜치) |
...
- **두레이 태스크 드로어 (⌘⇧T)** — 탭바 오른쪽 \`📋 태스크\` 로 열립니다. 업무 카드를 터미널 화면에 **끌어다 놓으면** 매핑된 저장소로 \`cd\` 하고 claude 가 실행되면서 업무 내용을 첫 지시로 전달합니다. 한 번 작업한 업무에는 \`세션 연결됨\` 배지가 붙고, 다음부터 드롭하면 \`claude --resume\` 으로 이전 대화를 이어갑니다(프롬프트 재입력 없음). 연결을 끊으려면 배지 옆 아이콘을 누르세요. 저장소는 ⚙ 설정 → 워크스페이스에서 등록합니다.
