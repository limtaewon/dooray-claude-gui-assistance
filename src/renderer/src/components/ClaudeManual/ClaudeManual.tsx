...
const SECTIONS: Section[] = [
  {
    id: 'terminal-scm', icon: GitBranch, title: '터미널 사이드 패널',
    content: `# 사이드 패널 — 업무 · 변경사항 · 히스토리 · 브랜치

기존 **브랜치 작업** 메뉴가 없어지고 그 기능이 **터미널 사이드 패널**로 들어왔습니다.
...
- **모든 브랜치** 토글로 전체 그래프를 봅니다. 브랜치·원격·태그만 보므로 스태시는 섞이지 않습니다.
- **더 보기** 로 50개씩 이어서 불러옵니다.

### 커밋 검색

검색창 아래 칩으로 **무엇을 기준으로 찾을지**를 고릅니다.
...
전부 **있는 그대로(리터럴)** 찾습니다 — \`feat(git):\` 처럼 괄호가 들어가도 그대로 검색됩니다.
검색 중에는 그래프가 접힙니다. 결과가 히스토리의 일부라 부모 커밋이 목록에 없어 선을 이을 수 없기 때문입니다.
...
| >_ | **터미널** | 내장 터미널 + 사이드 패널(업무 · 변경사항 · 히스토리 · 브랜치) |
...
- **사이드 패널 (⌘⇧T)** — 탭바 오른쪽 버튼으로 열립니다. \`업무\` 탭에서 업무 카드를 터미널 화면에 **끌어다 놓으면** 매핑된 저장소로 \`cd\` 하고 claude 가 실행되면서 업무 내용을 첫 지시로 전달합니다. 한 번 작업한 업무에는 \`세션 연결됨\` 배지가 붙고, 다음부터 드롭하면 \`claude --resume\` 으로 이전 대화를 이어갑니다(프롬프트 재입력 없음). 연결을 끊으려면 배지 옆 아이콘을 누르세요. 저장소는 ⚙ 설정 → 워크스페이스에서 등록합니다.
