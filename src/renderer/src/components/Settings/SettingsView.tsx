...
/** Anthropic API 키 오버라이드 — 패키징된 Electron 앱에서 키체인 접근 불가 시 우회 */
function AnthropicApiKeySettings(): JSX.Element {
  ...
  useEffect(() => {
    window.api.settings.get('anthropicApiKey').then((v) => {
  ...
  const clear = async (): Promise<void> => {
    setSaving(true)
    try {
      await window.api.settings.set('anthropicApiKey', '')
      setApiKey('')
      setExisting(false)
    } finally { setSaving(false) }
  }
  ...
  return (
    <div className="mt-8 p-4 rounded-xl bg-bg-surface border border-bg-border">
      <div className="mb-2">
        <h4 className="text-xs font-semibold text-text-primary flex items-center gap-1.5">
          <Key size={11} /> Anthropic API 키 (선택)
        </h4>
        <p className="text-[10px] text-text-tertiary mt-0.5">
          패키징된 앱에서 `claude` 로그인(키체인)이 동작하지 않아 "Not logged in" 오류가 뜨면
          ANTHROPIC_API_KEY를 입력하세요. 비워두면 기본 키체인 인증을 사용합니다.
        </p>
      </div>
      <div className="flex items-center gap-2">
        <div className="relative flex-1">
          <input
            ...
            placeholder="sk-ant-... (선택 사항)"
            className="w-full bg-bg-primary border border-bg-border rounded-md px-3 py-1.5 pr-8 text-xs text-text-primary placeholder:text-text-tertiary focus:outline-none focus:border-clover-blue"
          />
        ...
        <button onClick={save} disabled={saving}
          className="px-3 py-1.5 rounded-md bg-clover-blue text-white text-[11px] font-medium hover:bg-clover-blue/80 disabled:opacity-50">
          {saving ? '저장 중...' : '저장'}
        </button>
        {existing && (
          <button onClick={clear} disabled={saving}
            className="px-2 py-1.5 rounded-md bg-bg-primary text-text-secondary text-[11px] border border-bg-border hover:text-text-primary">
            삭제
          </button>
        )}
      </div>
      {saved && (
        <div className="mt-2 flex items-center gap-1 text-[10px] text-emerald-400">
          <Check size={10} /> 저장됨. AI 기능을 다시 시도해보세요.
        </div>
      )}
    </div>
  )
