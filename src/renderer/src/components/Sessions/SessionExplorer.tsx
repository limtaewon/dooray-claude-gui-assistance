        ...
        <div className="px-4 py-3 border-b border-bg-border flex-shrink-0">
          <div className="flex items-center justify-between mb-2">
            <div className="flex items-center gap-1.5">
              <MessageSquare size={15} className="text-brand-claude" />
              <h2 className="text-sm font-semibold text-text-primary">세션 탐색기</h2>
              <span className="text-[calc(11px_*_var(--app-font-scale,1))] text-text-tertiary">{filtered.length}/{sessions.length}</span>
            </div>
          ...
          <div className="relative mb-2">
            ...
            <input type="text" value={search} onChange={(e) => { setSearch(e.target.value); setRenderCount(50) }}
              ...
              className="w-full pl-7 pr-2 py-1.5 bg-bg-surface border border-bg-border rounded-lg text-xs text-text-primary placeholder-text-tertiary ds-focus" />
          </div>
          {/* 프로젝트 필터 */}
          <select value={projectFilter} onChange={(e) => { setProjectFilter(e.target.value); setRenderCount(50) }}
            className="w-full px-2 py-1 bg-bg-surface border border-bg-border rounded text-[calc(11px_*_var(--app-font-scale,1))] text-text-secondary ds-focus">
          ...
          {loading ? (
            ...
            <>
              ...
              {renderCount < filtered.length && (
                <div className="py-2 text-center text-[calc(11px_*_var(--app-font-scale,1))] text-text-tertiary">{visible.length}/{filtered.length} · 스크롤하면 더 표시</div>
              )}
            </>
          )}
        </div>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        {selectedSession ? (
            ...
            <div className="px-4 py-3 border-b border-bg-border flex-shrink-0">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <div className="flex items-center gap-3 mt-1 text-[calc(11px_*_var(--app-font-scale,1))] text-text-tertiary">
                ...
                <div className="flex gap-1.5">
                  <button onClick={summarizeSession} disabled={summarizing || !messages.length}
                    className="flex items-center gap-1 px-2.5 py-1 rounded-lg bg-bg-surface-raised border border-bg-border-light text-[calc(11px_*_var(--app-font-scale,1))] font-medium text-text-primary disabled:opacity-40">
                  ...
                  <button onClick={async () => {
                    ...
                    setCopied(true); setTimeout(() => setCopied(false), 2000)
                  }} className="flex items-center gap-1 px-2.5 py-1 rounded-lg bg-bg-active border border-bg-border-light text-[calc(11px_*_var(--app-font-scale,1))] text-clauday-orange hover:bg-bg-surface-hover"
              ...
              {summary && (
                <div className="mt-2 p-2.5 rounded-lg bg-bg-surface-raised border border-bg-border-light">
                  <div className="flex items-center gap-1 mb-1">
                    ...
                    <span className="text-[calc(11px_*_var(--app-font-scale,1))] font-semibold text-clauday-orange">AI 요약</span>
                  </div>
              ...
              {loadingDetail ? (
                ...
                <div key={i} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                  <div className={`max-w-[85%] rounded-xl px-3.5 py-2.5 text-xs leading-relaxed ${
                    m.role === 'user' ? 'bg-c-blue-solid text-white rounded-br-sm' : 'bg-bg-surface border border-bg-border text-text-primary rounded-bl-sm'
                  }`}>
          ...
          <div className="flex flex-col items-center justify-center h-full text-text-secondary gap-2">
            ...
            <p className="text-[calc(11px_*_var(--app-font-scale,1))] text-text-tertiary">검색으로 원하는 세션을 빠르게 찾을 수 있습니다</p>
          </div>
        )}
      </div>
    </div>
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-2">
        ...
        <span className="text-[calc(11px_*_var(--app-font-scale,1))] text-text-tertiary flex-shrink-0">{formattedTime}</span>
      </div>
      <div className="flex items-center gap-2 mt-1">
        <span className="text-[calc(11px_*_var(--app-font-scale,1))] text-text-tertiary flex items-center gap-0.5">
