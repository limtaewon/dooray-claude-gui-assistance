      ...
      <div onClick={(e) => e.stopPropagation()}
        ...
        <div className="flex items-center gap-3 px-5 py-4 border-b border-bg-border flex-shrink-0">
          ...
          <div className="flex-1">
            ...
            <p className="text-[calc(11px_*_var(--app-font-scale,1))] text-text-tertiary">AI가 자연어를 필터 규칙으로 변환해줍니다</p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-5">
            ...
            <div className="rounded-xl bg-bg-active border border-bg-border-light p-3 space-y-2">
              <div className="flex items-center justify-between mb-1">
                ...
                <button onClick={() => setFilter(null)}
                  className="text-[calc(11px_*_var(--app-font-scale,1))] text-text-tertiary hover:text-red-400">
                  규칙 비우기
                </button>
              </div>
              <div className="grid grid-cols-2 gap-2 text-[calc(11px_*_var(--app-font-scale,1))]">
                <EditableRuleSection label="포함 (OR)" items={filter.anyOf} color="text-emerald-400"
                  onChange={(items) => setFilter({ ...filter, anyOf: items })} />
                <EditableRuleSection label="모두 포함 (AND)" items={filter.allOf} color="text-brand-dooray"
              ...
              <p className="text-[calc(11px_*_var(--app-font-scale,1))] text-text-tertiary pt-1 border-t border-bg-border">
          ...
          <div>
            <div className="flex items-center justify-between mb-1.5">
              ...
              <span className="text-[calc(11px_*_var(--app-font-scale,1))] text-text-tertiary">{selectedIds.size}개 선택됨</span>
            </div>
            ...
            <div className="max-h-48 overflow-y-auto rounded-lg border border-bg-border divide-y divide-bg-border">
              {filteredChannels.length === 0 ? (
                <div className="px-3 py-4 text-[calc(11px_*_var(--app-font-scale,1))] text-text-tertiary text-center">채널이 없습니다</div>
...
function EditableRuleSection({ label, items, color, mono, onChange }: {
  ...
  return (
    <div>
      <div className={`text-[calc(11px_*_var(--app-font-scale,1))] font-bold mb-0.5 ${color}`}>{label}</div>
      <div className="flex flex-wrap gap-1 mb-1">
        {list.map((t, i) => (
          <span key={i}
            className={`group inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded text-[calc(11px_*_var(--app-font-scale,1))] bg-bg-surface border border-bg-border text-text-primary ${mono ? 'font-mono' : ''}`}>
            {t}
      ...
      <div className="flex items-center gap-1">
        <input
          ...
          placeholder="직접 추가"
          className={`flex-1 min-w-0 px-1.5 py-0.5 rounded text-[calc(11px_*_var(--app-font-scale,1))] bg-bg-surface border border-bg-border text-text-primary placeholder-text-tertiary focus:outline-none focus:border-clauday-orange ${mono ? 'font-mono' : ''}`}
        />
