  ...
  return (
    <>
      ...
      <div
        ...
        <div className="text-[calc(11px_*_var(--app-font-scale,1))] font-semibold text-text-tertiary uppercase tracking-wide mb-1.5">인기 색상</div>
        ...
        <div className="mt-3 pt-2 border-t border-bg-border">
          <div className="text-[calc(11px_*_var(--app-font-scale,1))] font-semibold text-text-tertiary uppercase tracking-wide mb-1.5">사용자 지정</div>
          <div className="flex items-center gap-1.5">
            ...
            <input
              ...
              placeholder="#RRGGBB"
              spellCheck={false}
              className={`flex-1 min-w-0 text-[calc(11px_*_var(--app-font-scale,1))] px-2 py-1 rounded bg-bg-surface-hover border ${invalid ? 'border-rose-500' : 'border-bg-border'} text-text-primary outline-none ds-focus font-mono`}
            />
          </div>
          {invalid && <div className="text-[calc(11px_*_var(--app-font-scale,1))] text-rose-400 mt-1">올바른 hex 값을 입력하세요 (예: #3b82f6)</div>}
          <div className="mt-1.5 text-[calc(11px_*_var(--app-font-scale,1))] text-text-tertiary">창을 닫으면 자동 적용됩니다</div>
        </div>

        {overridden && (
          <button
            onClick={onReset}
            className="mt-2 w-full text-[calc(11px_*_var(--app-font-scale,1))] text-text-tertiary hover:text-text-secondary border-t border-bg-border pt-1.5">
            기본 색상으로 되돌리기
          </button>
        )}
      </div>
  ...
  const renderItem = (c: CalEntry): JSX.Element => {
    ...
    return (
      <div key={c.id} className="relative w-full flex items-center gap-2 px-3 py-1.5 hover:bg-bg-surface-hover">
        ...
        {c.count > 0 && <span className="text-[calc(11px_*_var(--app-font-scale,1))] text-text-tertiary flex-shrink-0">{c.count}</span>}
        <button
          onClick={(e) => {
            e.stopPropagation()
        ...
        <>
          ...
          <div className="absolute left-0 top-full mt-1 w-64 bg-bg-surface border border-bg-border rounded-xl shadow-2xl z-40 overflow-hidden">
            <div className="px-3 py-2 border-b border-bg-border bg-bg-surface-hover">
              ...
              <span className="text-[calc(11px_*_var(--app-font-scale,1))] text-text-tertiary ml-2">{filterIds.length > 0 ? `${filterIds.length}개` : '전체'}</span>
            </div>
            <div className="max-h-72 overflow-y-auto py-1">
              {groups.local.length > 0 && (
                <>
                  <div className="px-3 pt-2 pb-1 text-[calc(11px_*_var(--app-font-scale,1))] font-semibold text-text-tertiary uppercase tracking-wide">Clauday</div>
                  {groups.local.map(renderItem)}
                </>
              )}
              {groups.caldav.length > 0 && (
                <>
                  <div className="px-3 pt-2 pb-1 text-[calc(11px_*_var(--app-font-scale,1))] font-semibold text-text-tertiary uppercase tracking-wide">두레이</div>
                  {groups.caldav.map(renderItem)}
                </>
              )}
              {/* 공휴일은 필터로 끌 수 없음 — 항상 표시되므로 필터 UI 에서 제외 */}
            </div>
            <div className="px-3 py-1.5 border-t border-bg-border text-[calc(11px_*_var(--app-font-scale,1))] text-text-tertiary">선택 없으면 전체 표시</div>
          </div>
        </>
      )}
    </div>
    ...
    <div className="h-full flex flex-col">
      ...
      <div className="flex items-center justify-between px-6 py-3 border-b border-bg-border flex-shrink-0">
        <div className="flex items-center gap-2">
          <CalendarDays size={18} className="text-brand-dooray" />
          <h2 className="text-lg font-semibold text-text-primary">이번 주 일정</h2>
          <span className="text-[calc(11px_*_var(--app-font-scale,1))] text-text-tertiary">{displayEvents.length}개{filterIds.length > 0 ? ` / ${events.length}` : ''}</span>
      ...
      <div className="mx-6 mt-3 flex items-center gap-2 flex-shrink-0">
        <ListTodo size={14} className="text-brand-dooray flex-shrink-0" />
        <input
          type="text"
      ...
      {caldavStatus && !caldavStatus.connected && (
        <div className="mx-6 mt-3 p-3 rounded-xl bg-bg-active border border-bg-border-light flex items-center gap-3 flex-shrink-0">
          ...
          <button
            ...
            className="px-2.5 py-1 rounded-md bg-brand-dooray text-white text-[calc(11px_*_var(--app-font-scale,1))] font-medium hover:opacity-90 flex-shrink-0">
            연결하러 가기
          </button>
        </div>
      )}
                      ...
                      <div
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-1.5 flex-wrap">
                            ...
                            {isOngoing && (
                              <span className="text-[calc(11px_*_var(--app-font-scale,1))] px-1.5 py-0.5 rounded-full bg-emerald-400/15 text-emerald-400 font-medium">진행 중</span>
                            )}
                            {isUpcoming && (
                              <span className="text-[calc(11px_*_var(--app-font-scale,1))] px-1.5 py-0.5 rounded-full bg-bg-active text-text-primary font-medium">예정</span>
                            )}
                          </div>
                          <div className="flex items-center gap-3 mt-0.5">
                            <div className="flex items-center gap-1 text-[calc(11px_*_var(--app-font-scale,1))] text-text-secondary">
