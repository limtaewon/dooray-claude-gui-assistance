 ...
 * `src/shared/git-history-log-parser.ts`. Copyright (c) 2026 Lovecast Inc. — MIT License.
 * 변경: 없음 (verbatim).
 */
import type { GitHistoryItem, GitHistoryItemRef } from './historyTypes'
import { iterateNulDelimitedFields } from './nulFields'
...
function parseGitDecorationRefs(raw: string, revision: string): GitHistoryItemRef[] {
  ...
  // git 은 ref 이름에 콤마를 허용하므로 콤마로 자르면 깨진다 — ref 에 들어갈 수 없는
  // 제어문자(\x1f)를 구분자로 쓰고, 옛 포맷 호환으로 콤마 분할을 남긴다.
  ...
  for (const part of parts) {
    ...
    if (ref.startsWith('refs/heads/')) {
      refs.push({ id: ref, name: ref.slice('refs/heads/'.length), revision, category: 'branches' })
      continue
    }
